...
<div class="box box-widget">
	<div class="box-header with-border">
		<div class="content-header">
			<h1><i class="fa {{$module->fa_icon}}"></i> NHÓM KHÁCH HÀNG<small>| Danh sách nhóm khách hàng</small></h1>
			<span class="headerElems">
			@la_access("CustomerGroups", "create")
				<button class="btn btn-success btn-sm pull-right" id="AddBtn"><i class="fa fa-plus-square"></i>&nbsp;&nbsp;Thêm nhóm khách hàng mới</button>
			@endla_access
			</span>
		</div>
	</div>
...
@endla_access
@la_access("CustomerGroups", "delete")
<div class="modal fade" id="DeleteModal" role="dialog">
	<div class="modal-dialog" role="document">
		<div id="DeleteModalContent" class="modal-content">
			<div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
				<h1><i class="fa fa-trash"></i> XÓA<small>| Xóa nhóm khách hàng</small></h1>
            </div>
			{!! Form::open(['route' => [config('laraadmin.adminRoute') . '.customergroups.destroy', 1], 'method'=>'delete', 'id' => 'customergroup-delete-form']) !!}
			<div class="modal-body">
                Bạn có chắc chắn muốn xóa <b><span class="name-delete text-red"></span></b> khỏi danh sách nhóm khách hàng không?
            </div>
...
$(function () {
	$("#example1").DataTable({
		processing: true,
        serverSide: true,
        ajax: "{{ url(config('laraadmin.adminRoute') . '/customergroup_dt_ajax') }}",
		language: {
...
$("#AddBtn").click(function() {
	$.ajax({ url: "{{ url(config('laraadmin.adminRoute') . '/customergroup_fm_add') }}" }).done(function(htmlcode) {
		$('#AddModalContent').html(htmlcode);
		loadUI();
		$('#AddModal').modal('show');
		$("#customergroup-add-form").validate({
...
function ResponseAdd(responseText, statusText, xhr, $form)  {
	$("#customergroup-add-form").validate().resetForm();		
	$('#AddModal').modal('hide');
	$('#example1').DataTable().ajax.reload();
	toastr.success('', 'Đã thêm nhóm khách hàng mới.', {positionClass: "toast-bottom-right", timeOut: "3000"}).css({ "width":"auto", "max-width":"100em" });
}
// Edit Process
function EditProcess(url){
	$.ajax({ url: url }).done(function(htmlcode) {
		...
		$("#customergroup-edit-form").validate({
			submitHandler: function(form) {
...
function DeleteProcess(url, name){
	$('#customergroup-delete-form').attr('action', url);
	$('.name-delete').html(name);
	$('#DeleteModal').modal('show');
	$("#customergroup-delete-form").validate({
		submitHandler: function(form) {
