...
<div class="box box-widget">
	<div class="box-header with-border">
		<div class="content-header">
			<h1><i class="fa {{$module->fa_icon}}"></i> Employees<small>| Employees listing</small></h1>
			<span class="headerElems">
			@la_access("Employees", "create")
				<button class="btn btn-success btn-sm pull-right" id="AddBtn"><i class="fa fa-plus-square"></i>&nbsp;&nbsp;Thêm Employee mới</button>
			@endla_access
			</span>
		</div>
	</div>
	...
	<div class="modal-dialog" role="document">
		<div id="DeleteModalContent" class="modal-content">
			<div class="modal-header">
				...
				<h1><i class="fa fa-trash"></i> XÓA<small>| Xóa Employee</small></h1>
            </div>
			{!! Form::open(['route' => [config('laraadmin.adminRoute') . '.employees.destroy', 1], 'method'=>'delete', 'id' => 'employee-delete-form']) !!}
			<div class="modal-body">
                Bạn có chắc chắn muốn xóa <b><span class="name-delete text-red"></span></b> khỏi Employees listing không?
            </div>
...
$("#AddBtn").click(function() {
	$.ajax({ url: "{{ url(config('laraadmin.adminRoute') . '/employee_fm_add') }}" }).done(function(htmlcode) {
...
function ResponseAdd(responseText, statusText, xhr, $form)  {
	$("#employee-add-form").validate().resetForm();		
	$('#AddModal').modal('hide');
...
function DeleteProcess(url, name){
	$('#employee-delete-form').attr('action', url);
