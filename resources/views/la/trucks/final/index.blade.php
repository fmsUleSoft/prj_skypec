...
<div class="box box-widget">
	<div class="box-header with-border">
		<div class="content-header">
			<h1><i class="fa {{$module->fa_icon}}"></i> Trucks<small>| Trucks listing</small></h1>
			<span class="headerElems">
			@la_access("Trucks", "create")
				<button class="btn btn-success btn-sm pull-right" id="AddBtn"><i class="fa fa-plus-square"></i>&nbsp;&nbsp;Thêm Truck mới</button>
			@endla_access
			</span>
		</div>
	</div>
	...
	<div class="modal-dialog" role="document">
		<div id="DeleteModalContent" class="modal-content">
			<div class="modal-header">
				...
				<h1><i class="fa fa-trash"></i> XÓA<small>| Xóa Truck</small></h1>
            </div>
			{!! Form::open(['route' => [config('laraadmin.adminRoute') . '.trucks.destroy', 1], 'method'=>'delete', 'id' => 'truck-delete-form']) !!}
			<div class="modal-body">
                Bạn có chắc chắn muốn xóa <b><span class="name-delete text-red"></span></b> khỏi Trucks listing không?
            </div>
...
$("#AddBtn").click(function() {
	$.ajax({ url: "{{ url(config('laraadmin.adminRoute') . '/truck_fm_add') }}" }).done(function(htmlcode) {
