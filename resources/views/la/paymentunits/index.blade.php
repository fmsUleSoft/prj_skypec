...
<div class="box box-widget">
	<div class="box-header with-border">
		<div class="content-header">
			<h1><i class="fa {{$module->fa_icon}}"></i> ĐƠN VỊ THANH TOÁN<small>| Danh sách đơn vị thanh toán</small></h1>
			<span class="headerElems">
			@la_access("PaymentUnits", "create")
				<button class="btn btn-success btn-sm pull-right" id="AddBtn"><i class="fa fa-plus-square"></i>&nbsp;&nbsp;Thêm đơn vị thanh toán mới</button>
			@endla_access
			</span>
		</div>
	</div>
...
@endla_access
@la_access("PaymentUnits", "delete")
<div class="modal fade" id="DeleteModal" role="dialog">
	<div class="modal-dialog" role="document">
		<div id="DeleteModalContent" class="modal-content">
			<div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
				<h1><i class="fa fa-trash"></i> XÓA<small>| Xóa đơn vị thanh toán</small></h1>
            </div>
			{!! Form::open(['route' => [config('laraadmin.adminRoute') . '.paymentunits.destroy', 1], 'method'=>'delete', 'id' => 'paymentunit-delete-form']) !!}
			<div class="modal-body">
                Bạn có chắc chắn muốn xóa <b><span class="name-delete text-red"></span></b> khỏi danh sách đơn vị thanh toán không?
            </div>
...
$(function () {
	$("#example1").DataTable({
		processing: true,
        serverSide: true,
        ajax: "{{ url(config('laraadmin.adminRoute') . '/paymentunit_dt_ajax') }}",
		language: {
...
$("#AddBtn").click(function() {
	$.ajax({ url: "{{ url(config('laraadmin.adminRoute') . '/paymentunit_fm_add') }}" }).done(function(htmlcode) {
		$('#AddModalContent').html(htmlcode);
		loadUI();
		$('#AddModal').modal('show');
		$("#paymentunit-add-form").validate({
...
function ResponseAdd(responseText, statusText, xhr, $form)  {
	$("#paymentunit-add-form").validate().resetForm();		
	$('#AddModal').modal('hide');
	$('#example1').DataTable().ajax.reload();
	toastr.success('', 'Đã thêm đơn vị thanh toán mới.', {positionClass: "toast-bottom-right", timeOut: "3000"}).css({ "width":"auto", "max-width":"100em" });
}
// Edit Process
function EditProcess(url){
	$.ajax({ url: url }).done(function(htmlcode) {
		...
		$("#paymentunit-edit-form").validate({
			submitHandler: function(form) {
...
function DeleteProcess(url, name){
	$('#paymentunit-delete-form').attr('action', url);
	$('.name-delete').html(name);
	$('#DeleteModal').modal('show');
	$("#paymentunit-delete-form").validate({
		submitHandler: function(form) {
